...
impl TCP {
    ...
    fn receive_handler(&self) -> Result<()> {
        ...
        let (_, mut receiver) = transport::transport_channel(
            65535,
            TransportChannelType::Layer3(IpNextHeaderProtocols::Tcp), // IP Address が必要なので IP パケットレベルで取得
        )
        .unwrap();
...
/// iproute2-ssl80129 で動作を確認。バージョンによって挙動が変わるかも
...
struct TCPEvent {
    sock_id: SockID, // イベント発生元の Socket ID
    kind: TCPEventKind,
}
